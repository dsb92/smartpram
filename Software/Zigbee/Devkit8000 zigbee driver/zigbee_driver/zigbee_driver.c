...
{
	uint8 temp;
	if ( (CCA_mode > 3) || (CCA_mode < 1) )	// Check for invalid value
		CCA_mode = 1;						// Default CCA-mode set to mode 1 (2)
...
{
	if ( RSSI_mode > 1 )						// Check for invalid value
		RSSI_mode = 1;						// Default RSSI-mode, append on
