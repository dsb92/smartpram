...
CY_ISR(isr_timer_Interrupt)
{
    ...
    /* PSoC3 ES1, ES2 RTC ISR PATCH  */ 
        ...
        #if((CYDEV_CHIP_REVISION_USED <= CYDEV_CHIP_REVISION_3A_ES2) && (isr_timer__ES2_PATCH ))      
            isr_timer_ISR_PATCH();
...
{
    CY_SET_REG16(isr_timer_INTC_VECTOR, (uint16) address);
}
...
{
    return (cyisraddress) CY_GET_REG16(isr_timer_INTC_VECTOR);
}
...
uint8 isr_timer_GetState(void) 
{
    ...
    return ((*isr_timer_INTC_SET_EN & (uint8)isr_timer__INTC_MASK) != 0u) ? 1u:0u;
}
